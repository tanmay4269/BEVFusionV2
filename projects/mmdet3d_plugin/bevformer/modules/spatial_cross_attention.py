...
class SpatialCrossAttention(BaseModule):
    ...
    def __init__(self,
        ...
        if self.use_camera:
            ...
            self.img_deformable_attention = build_attention(deformable_attention)

        if self.use_lidar:
            self.pts_output_proj = nn.Linear(embed_dims, embed_dims)
            self.pts_deformable_attention = build_attention(deformable_attention)
    ...
    def forward(self,
        ...
        if self.use_camera:
            ...
            num_cams, l, bs, embed_dims = img_feats_flatten.shape

            value = img_feats_flatten.permute(2, 0, 1, 3).reshape(
                bs * self.num_cams, l, self.embed_dims)

            queries = self.img_deformable_attention(
            ...
            slots = self.img_output_proj(slots)

            img_output = self.dropout(slots) + inp_residual

        if self.use_lidar:
            ref_points = reference_points.permute(0, 2, 1, 3)
            ref_points = ref_points[..., :2]
            tmp_output = self.pts_deformable_attention(
                query=query,
                value=pts_feats_flatten,
                reference_points=ref_points,
                spatial_shapes=pts_spatial_shapes,
                level_start_index=level_start_index
            )

            pts_output = self.pts_output_proj(tmp_output)
