...
class PerceptionTransformerBEVEncoder(BaseModule):
    def __init__(self,
                 ...
                 use_unifed_queries=True,
                 **kwargs):
        super(PerceptionTransformerBEVEncoder, self).__init__(**kwargs)
        self.use_unifed_queries = use_unifed_queries
    ...
    def forward(self,
        ...
        img_spatial_shapes = torch.as_tensor(img_spatial_shapes, dtype=torch.long, device=bev_pos.device)
        level_start_index = torch.cat((img_spatial_shapes.new_zeros((1,)), img_spatial_shapes.prod(1).cumsum(0)[:-1]))
        ...
        if mlvl_pts_feats is not None:
            ...
            pts_feat_flatten = torch.cat(pts_feat_flatten, 1)
            pts_spatial_shapes = torch.as_tensor(
                pts_spatial_shapes, dtype=torch.long, device=pts_feat_flatten.device)
        
            # TODO: Use valid ratio 
...
class PerceptionTransformerV2(PerceptionTransformerBEVEncoder):
    ...
    def __init__(self,
        ...
        super(PerceptionTransformerV2, self).__init__(num_feature_levels, num_cams, two_stage_num_proposals, encoder,
                                                      embed_dims, use_cams_embeds, rotate_center, use_unifed_queries,
                                                      **kwargs)
    ...
    def forward(self,
        ...
        Args:
            mlvl_img_feats (list(Tensor)): Input queries from
